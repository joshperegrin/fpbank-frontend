    ...
    <ion-content class="ion-padding">
      <div class="content-container">
        ...
        <div class="ion-activatable transfer" @click="set">
          ...
          <div class="selectDestination" :style="(this.servicesStore.serviceDetails.transfer_ReceivingAccountNumber && this.servicesStore.serviceDetails.transfer_ReceivingAccountNumber.trim() !== '')? { borderBottom: '1px solid var(--ion-color-lightmedium)', marginBottom: '8px' } : {}"><span>Transfer To</span> <ion-icon :icon="this.chevronForwardOutline"></ion-icon></div>
          <div v-if="(this.servicesStore.serviceDetails.transfer_ReceivingAccountNumber && this.servicesStore.serviceDetails.transfer_ReceivingAccountNumber.trim() !== '')"> FP BANK ACCOUNT </div>
        ...
        <ion-input ref="amountTextbox" :disabled="!(this.servicesStore.serviceDetails.transfer_ReceivingAccountNumber && this.servicesStore.serviceDetails.transfer_ReceivingAccountNumber.trim() !== '')" label="Amount*" type="number" inputmode="decimal" step="0.01" min="0"  v-model="amount" @input="validateInput" label-placement="stacked" fill='outline'></ion-input>
...
export default {
  ...
  methods: {
    async handleBackButtonClick(){
      if(this.servicesStore.serviceDetails.transfer_ReceivingAccountNumber !== '' ||
    ...
    set(){
      ...
      this.$router.push(`/tabs/tab2/transfer/internal/destinationSelect`);
    },
    handleClick() {
      const bank = this.servicesStore.serviceDetails.transfer_ReceivingBank;
      if (bank && bank.trim() !== '') {
        this.$router.push('/tabs/tab2/transfer/internal/transferChannel');
      }
    },
    async confirm(){
      if(this.isFormComplete()){
        ...
        this.$router.push('/tabs/tab2/transfer/internal/confirmation');
